...
class DatedDashboardComponent extends Component
{
    ...
    public function mount()
    {
        ...
        $this->co_drama = Worker::where('department', 'Theatre')->count();
        ...
        $this->co_pcu = Worker::where('department', 'Pastoral')->count();
        ...
        $this->co_ts = Worker::where('department', 'Security/Traffic')->count();
        ...
        if (!$this->date && !is_null($this->oldDate)) {
            ...
            $this->earl_drama = Attendance::where('department', 'Theatre')->where('status', 'Early')->where('created_at', 'LIKE', '%' . Carbon::createFromFormat('Y-m-d H:i:s', $this->oldDate)->format('Y-m-d') . '%')->count();
            $this->late_drama = Attendance::where('department', 'Theatre')->where('status', 'Late')->where('created_at', 'LIKE', '%' . Carbon::createFromFormat('Y-m-d H:i:s', $this->oldDate)->format('Y-m-d') . '%')->count();
            $this->abs_drama = Attendance::where('department', 'Theatre')->where('status', 'Absent')->where('created_at', 'LIKE', '%' . Carbon::createFromFormat('Y-m-d H:i:s', $this->oldDate)->format('Y-m-d') . '%')->count();
            ...
            $this->earl_pcu = Attendance::where('department', 'Pastoral')->where('status', 'Early')->where('created_at', 'LIKE', '%' . Carbon::createFromFormat('Y-m-d H:i:s', $this->oldDate)->format('Y-m-d') . '%')->count();
            $this->late_pcu = Attendance::where('department', 'Pastoral')->where('status', 'Late')->where('created_at', 'LIKE', '%' . Carbon::createFromFormat('Y-m-d H:i:s', $this->oldDate)->format('Y-m-d') . '%')->count();
            $this->abs_pcu = Attendance::where('department', 'Pastoral')->where('status', 'Absent')->where('created_at', 'LIKE', '%' . Carbon::createFromFormat('Y-m-d H:i:s', $this->oldDate)->format('Y-m-d') . '%')->count();
            ...
            $this->earl_ts = Attendance::where('department', 'Security/Traffic')->where('status', 'Early')->where('created_at', 'LIKE', '%' . Carbon::createFromFormat('Y-m-d H:i:s', $this->oldDate)->format('Y-m-d') . '%')->count();
            $this->late_ts = Attendance::where('department', 'Security/Traffic')->where('status', 'Late')->where('created_at', 'LIKE', '%' . Carbon::createFromFormat('Y-m-d H:i:s', $this->oldDate)->format('Y-m-d') . '%')->count();
            $this->abs_ts = Attendance::where('department', 'Security/Traffic')->where('status', 'Absent')->where('created_at', 'LIKE', '%' . Carbon::createFromFormat('Y-m-d H:i:s', $this->oldDate)->format('Y-m-d') . '%')->count();
        ...
        ///////////////////////////////
            ...
            $this->earl_drama = Attendance::where('department', 'Theatre')->where('status', 'Early')->where('created_at', 'LIKE', '%' .  $this->date . '%')->count();
            $this->late_drama = Attendance::where('department', 'Theatre')->where('status', 'Late')->where('created_at', 'LIKE', '%' .  $this->date . '%')->count();
            $this->abs_drama = Attendance::where('department', 'Theatre')->where('status', 'Absent')->where('created_at', 'LIKE', '%' .  $this->date . '%')->count();
            ...
            $this->earl_pcu = Attendance::where('department', 'Pastoral')->where('status', 'Early')->where('created_at', 'LIKE', '%' .  $this->date . '%')->count();
            $this->late_pcu = Attendance::where('department', 'Pastoral')->where('status', 'Late')->where('created_at', 'LIKE', '%' .  $this->date . '%')->count();
            $this->abs_pcu = Attendance::where('department', 'Pastoral')->where('status', 'Absent')->where('created_at', 'LIKE', '%' .  $this->date . '%')->count();
            ...
            $this->earl_ts = Attendance::where('department', 'Security/Traffic')->where('status', 'Early')->where('created_at', 'LIKE', '%' .  $this->date . '%')->count();
            $this->late_ts = Attendance::where('department', 'Security/Traffic')->where('status', 'Late')->where('created_at', 'LIKE', '%' .  $this->date . '%')->count();
            $this->abs_ts = Attendance::where('department', 'Security/Traffic')->where('status', 'Absent')->where('created_at', 'LIKE', '%' .  $this->date . '%')->count();
    ...
    public function sendReport()
    {
        ...
        if (!$this->date && !is_null($this->oldDate)) {
            ...
            Mail::to('eniwo1@gmail.com')
                // ->cc('omoalu@outlook.com')
                // ->bcc('davemoses3@gmail.com')
                ->queue(new SendReport($body));

            $deptBodys = [
                    ['email' => 'amosesharive@gmail.com', 'data' => ['name' => 'Amos Esharive', 'dept' => 'Avalanche', 'co_members' => $this->co_aval, 'co_early' => $this->earl_aval, 'co_late' => $this->late_aval, 'co_absent' => $this->abs_aval, 'link' => url('/attendance/report/Avalanche/' . $this->date), 'date' => $this->date]],
                    ['email' => 'denedofaith88@gmail.com', 'data' => ['name' => 'Faith Denedo', 'dept' => 'Admin', 'co_members' => $this->co_admin, 'co_early' => $this->earl_admin, 'co_late' => $this->late_admin, 'co_absent' => $this->abs_admin, 'link' => url('/attendance/report/Admin/' . $this->date), 'date' => $this->date]],
                    ['email' => 'eniwo1@gmail.com', 'data' => ['name' => 'ENIWAREKE	DESMOND', 'dept' => 'Admin', 'co_members' => $this->co_admin, 'co_early' => $this->earl_admin, 'co_late' => $this->late_admin, 'co_absent' => $this->abs_admin, 'link' => url('/attendance/report/Admin/' . $this->date), 'date' => $this->date]],
                    ['email' => 't879643@gmail.com', 'data' => ['name' => 'Godstime Emiko', 'dept' => 'Admin', 'co_members' => $this->co_admin, 'co_early' => $this->earl_admin, 'co_late' => $this->late_admin, 'co_absent' => $this->abs_admin, 'link' => url('/attendance/report/Admin/' . $this->date), 'date' => $this->date]],
                    ['email' => 'hericmaakpede90@gmail.com', 'data' => ['name' => 'Godswill Akpede', 'dept' => 'Sanctuary', 'co_members' => $this->co_spark, 'co_early' => $this->earl_spark, 'co_late' => $this->late_spark, 'co_absent' => $this->abs_spark, 'link' => url('/attendance/report/Sparkles/' . $this->date), 'date' => $this->date]],
                    ['email' => 'richardufuoma802@gmail.com', 'data' => ['name' => 'Ufuoma	Obote', 'dept' => 'Sanctuary', 'co_members' => $this->co_spark, 'co_early' => $this->earl_spark, 'co_late' => $this->late_spark, 'co_absent' => $this->abs_spark, 'link' => url('/attendance/report/Sparkles/' . $this->date), 'date' => $this->date]],
                    ['email' => 'godwinjose53@gmail.com', 'data' => ['name' => 'Godwin	Joseph', 'dept' => 'Theatre', 'co_members' => $this->co_drama, 'co_early' => $this->earl_drama, 'co_late' => $this->late_drama, 'co_absent' => $this->abs_drama, 'link' => url('/attendance/report/Drama/' . $this->date), 'date' => $this->date]],
                    ['email' => 'felixmakava@gmail.com', 'data' => ['name' => 'Felix Makava', 'dept' => 'Theatre', 'co_members' => $this->co_drama, 'co_early' => $this->earl_drama, 'co_late' => $this->late_drama, 'co_absent' => $this->abs_drama, 'link' => url('/attendance/report/Drama/' . $this->date), 'date' => $this->date]],
                    ['email' => 'oghenekarovictoroteri1@gmail.com', 'data' => ['name' => 'Oghenekaro Oteri', 'dept' => 'Media', 'co_members' => $this->co_media, 'co_early' => $this->earl_media, 'co_late' => $this->late_media, 'co_absent' => $this->abs_media, 'link' => url('/attendance/report/Media/' . $this->date), 'date' => $this->date]],
                    ['email' => 'kudedinihu@gmail.com', 'data' => ['name' => 'Kelechi Udedinihu', 'dept' => 'Media', 'co_members' => $this->co_media, 'co_early' => $this->earl_media, 'co_late' => $this->late_media, 'co_absent' => $this->abs_media, 'link' => url('/attendance/report/Media/' . $this->date), 'date' => $this->date]],
                    ['email' => 'desmondeberu@gmail.com', 'data' => ['name' => 'David Eberu', 'dept' => 'Pastoral', 'co_members' => $this->co_pcu, 'co_early' => $this->earl_pcu, 'co_late' => $this->late_pcu, 'co_absent' => $this->abs_pcu, 'link' => url('/attendance/report/Pastoral Care/' . $this->date), 'date' => $this->date]],
                    ['email' => 'eniwo1@gmail.com', 'data' => ['name' => 'ENIWAREKE	DESMOND', 'dept' => 'Pastoral', 'co_members' => $this->co_pcu, 'co_early' => $this->earl_pcu, 'co_late' => $this->late_pcu, 'co_absent' => $this->abs_pcu, 'link' => url('/attendance/report/Pastoral Care/' . $this->date), 'date' => $this->date]],
                    ['email' => 'adedeaugustine@gmail.com', 'data' => ['name' => 'Adede	Augustine', 'dept' => 'Security/Traffic', 'co_members' => $this->co_ts, 'co_early' => $this->earl_ts, 'co_late' => $this->late_ts, 'co_absent' => $this->abs_ts, 'link' => url('/attendance/report/Traffic & Security/' . $this->date), 'date' => $this->date]],
                    ...
                    // ['email' => 'progressigben24@gmail.com', 'data' => ['name' => 'IGBEN	PROGRESS', 'dept' => 'Hospitality', 'co_members' => $this->co_hosp, 'co_early' => $this->earl_hosp, 'co_late' => $this->late_hosp, 'co_absent' => $this->abs_hosp, 'link' => url('/attendance/report/Hospitality/' . $this->date), 'date' => $this->date]],
